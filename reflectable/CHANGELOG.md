...
* Change `build.yaml` such that reflectable is automatically scheduled by
  the build runner to be the last code generator. This ensures that the
  reflectable generator gets to see all the libraries/parts that are
  generated by other code generators, which is required in order to ensure
  that reflection support is generated for all applicable parts of the
  program.
